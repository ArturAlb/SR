...
ONION_ADDRESS=$(docker exec sr-tor-proxy-1 cat /var/lib/tor/hidden_service/hostname 2>/dev/null)

# Check if the onion address was retrieved successfully
if [ -z "$ONION_ADDRESS" ]; then
    echo "ERROR: Failed to get .onion address from Tor proxy."
    echo "Make sure:"
    echo "1. The 'sr-tor-proxy-1' container is running"
...
docker exec sr-client-1 curl --socks5-hostname 172.18.0.20:9050 -v "http://$ONION_ADDRESS"
